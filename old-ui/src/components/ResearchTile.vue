<template>
  <g :transform="`translate(0, ${y})`" v-b-tooltip.html.left :title="tooltip" :class="field">
    ...
    <Resource
      ...
      :key="'field-' + i"
      :transform="`translate(${2 + 56 / 2 + resourceX(i)}, ${(height / 3) * 2 + 3})`"
      :kind="resource.type"
    ...
    <Token
      ...
      :transform="`translate(${tokenX(player.player)}, ${tokenY(player.player)})`"
      :key="player.player"
      :scale="5.5"
    />
    <FederationTile
      v-if="this.federation"
    ...
    />
    <circle
      v-if="this.lostPlanet"
      :class="['planet-fill', this.lostPlanet]"
      cx="30"
      cy="18"
      r="10"
      style="pointer-events: none"
...
@Component<ResearchTile>({
  computed: {
    players(): Player[] {
      return this.$store.state.data.players.filter(
    ...
    federation(): Federation {
      if (this.level === 5) {
        if (this.field === ResearchField.Terraforming) {
          return this.$store.state.data.terraformingFederation;
        }
      }
    },

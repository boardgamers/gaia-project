...
import Vue from "vue";
import { makeStore } from "../../viewer/src/store";
import Game from "./components/Game.vue";
import ResearchTile from "./components/ResearchTile.vue";
import TechTile from "./components/TechTile.vue";

Vue.use(BootstrapVue);

describe("old UI move compatibility", () => {
  it("sends current controller commands from the old technology and research tiles", () => {
    const store = makeStore();
    store.state.context.highlighted.techs.add("terra" as any);
    store.state.context.highlighted.researchTiles.add("terra-1");
    const dispatch = vi.spyOn(store, "dispatch").mockResolvedValue(undefined);
    const tech = new TechTile({ store, propsData: { pos: "terra" } });
    const research = new ResearchTile({ store, propsData: { field: "terra", level: 1 } });
    try {
      tech.onClick();
      research.onClick();
      expect(dispatch.mock.calls).to.deep.equal([
        ["techClick", { command: "terra" }],
        ["researchClick", { command: "terra" }],
      ]);
    } finally {
      tech.$destroy();
      research.$destroy();

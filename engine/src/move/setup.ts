...
import { BoardAction, Command, Faction, Phase, Player as PlayerEnum } from "../enums";
...
/** Old games drew before bans, so banning a drawn faction could leave the last seat without a pick. */
export function repairBannedRandomFactions(engine: Engine): boolean {
  if (
    engine.phase !== Phase.SetupFaction ||
    engine.isVersionOrLater("4.14.3") ||
    !engine.options.randomFactions ||
  ...
  const banned = engine.randomFactions.filter((faction) => engine.bannedFactions.includes(faction));
  if (!banned.length || banned.some((faction) => engine.setup.includes(faction))) {
    return false;
  }

  const retained = engine.randomFactions.filter((faction) => !banned.includes(faction));
  const random = seedrandom(`${engine.moveHistory[0]}:replace-banned-random-factions`);
  const replacements = banned.map(() => {
    const possible = difference(remainingFactions(retained, engine.expansions), engine.bannedFactions);
    assert(possible.length > 0, "Not enough unbanned faction colours for all players");
    const faction = possible[Math.floor(possible.length * random())];
    retained.push(faction);
    return faction;
  });
  engine.randomFactions = engine.randomFactions.map((faction) =>
    banned.includes(faction) ? replacements[banned.indexOf(faction)] : faction
  );
  // Saved states may still contain the empty command list generated by the old pool.
  engine.availableCommands = undefined;
  return true;
}

...
describe("finalRankings", () => {
  it("uses the Lost Fleet neutral values in 2-player games", () => {
    const players = [new Player(Expansion.LostFleet, PlayerEnum.Player1), new Player(Expansion.LostFleet, PlayerEnum.Player2)];

    expect(finalRankings([FinalTile.Asteroid], players, Expansion.LostFleet)[0].map((entry) => entry.count)).to.deep.equal([
      3, 0, 0,
    ]);
    expect(
    ...
    expect(finalRankings([FinalTile.PlanetType], players, Expansion.LostFleet)[0].map((entry) => entry.count)).to.deep.equal([
      6, 0, 0,
    ]);
    expect(finalRankings([FinalTile.DeepSpaceSector], players, Expansion.LostFleet)[0].map((entry) => entry.count)).to.deep.equal([
      3, 0, 0,
    ]);
  });

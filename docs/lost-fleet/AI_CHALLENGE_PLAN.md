...
1. **Pull the latest `master` of `kimphamnguyen/gaia-lost-fleet` from GitHub and work against
   ...
   _current_ engine and viewer. An older checkout (or reasoning from memory) will describe code
...
`master`). The setup is locked in §2 and validated; **nothing is coded yet**, so review the _design_,
the _feature list_ (§6–§7), the _milestones_ (§9), and the _open questions_ (§10) — and push back
...
need an AI that plays _any_ Gaia game well; we need one that plays **this one game** extremely well.
...
* Lost Fleet: 3 ships (2 shuttles), ship techs {tfmars, eclipse}, ship feds {twilight, tfmars, eclipse},
  2 artifact tokens.
* **Central tension:** the board is Gaia-heavy (two mine-on-Gaia rounds + the Gaia final) _and_
...
- **Order = turn order.** Gaia's initial turn order derives from setup pick order, so the listed
  sequence _is_ the turn order (unless the owner specifies otherwise).
...
**Chosen format: 2-player (human vs one AI)** — committed. For a _strongest-possible-AI_ skill
...
**Free replayability:** seed-locked self-play trains _one_ net that plays _every_ seat, so letting the
...
- **Legal moves are cleanly exposed** via `engine.generateAvailableCommandsIfNeeded()` →
  `AvailableCommand[]`, each with structured `.data`. The branching factor lives _inside_ `.data`
...
| ---------------- | -------------------------------------------- | ----------------------------------------------- |
| `ai/features.ts` | engine state → numeric feature vector        | encodes _what to look at_ (see §7)              |
...
**Reframe that governs everything:** a faction/position's value is _not_ a fixed number — it is the
expected result of the game that follows under good play. So faction choice and every in-game move
are the _same_ search problem; `ChooseFaction` is just an early node. With the setup fixed, we
...
- **Owner-locked maximum mode (2026-07-13):** every supported device runs the same model/book and
  fixed high simulation workload. Do not shorten search or substitute a smaller model for a slow
  device; declare incapable devices unsupported. Faster hardware changes waiting time, not the
  configured opponent. The later UI must show real search progress, a moving ETA range, and a
  worker heartbeat, and must report browser background suspension distinctly.
...
1. **Warm-start / fine-tuning (biggest practical win).** Don't train each month from scratch.
   ...
   _re-specialize_ instead of re-learning Gaia. Turns each month's training from days into hours and
...
4. **Human-game flywheel (the compounding advantage).** Every month's challenge generates real human
   ...
   _humans specifically_.

**Data-reuse nuance (be precise):** raw self-play _games_ from month A are on board A, so they are
**not** direct training data for month B's _specialized head_ — but they are exactly the fuel for the
...
**Caveat — negative transfer:** warm-starting from _last month's over-specialized_ net can carry
...
The first N plies are the _same tree every game_. Analyze them exhaustively offline, store best
responses in a table keyed by the move-history prefix. Instant, near-perfect openings; live search
is saved for the midgame. Exists _only because_ the setup is fixed.
...
Predict every seat's outcome, not a single scalar — the correct formulation _if_ you ever add a 3–4p
...
one fixed _asymmetric_ layout, so there are no symmetric equivalents to augment with. Skip it.
...
- **A much deeper opening book (§6.1).** One opponent → a far narrower opening tree, so the fixed-seed
  book pre-solves _much_ more of the game for the same storage/compute.
...
- **Clean best-response / human exploitation.** With exactly one opponent to model, the human-game
  ...
  This makes the AI specifically hard for _humans_ to beat.
- **More valuable pondering (§6.2).** Only one opponent's move to anticipate; predict it and
  pre-search the reply precisely.

Net effect: 2p lets you _drop_ multiplayer complexity (max-n, kingmaking, per-player value) **and**
...
- **The margin/zero-sum value (§6.7) sets the threshold automatically.** margin = my score −
  ...
  costly spite no" line _is_ the margin calculation — not a hand-tuned threshold.
  - **Do NOT add a separate "hurt the opponent" bonus.** Margin already counts opponent losses; an
    ...
    cost/benefit; this _is_ the "don't ruin it for yourself" guardrail, and it's automatic as long as
    denial isn't over-encoded.
- **The opponent model (§6.7) makes the AI _see_ what to deny.** Search only blocks a plan whose
...
**Why this needs the human model specifically:** self-play trains against a _strong, flexible_
opponent that rarely telegraphs a fat, deniable single-track plan — so a purely self-play AI tends to
_under_-value denial against humans. Humans commit to predictable plans that _can_ be cheaply
...
per-round action first, take the tile they're racing for); the new part is predicting the _softer,
non-obvious_ targets.
...
| ------------------------------------------------------------------- | ------------------------ | ---------------------- |
| _What matters_ (leech potential, round-scoring alignment, blocking) | `ai/features.ts`         | state → numbers        |
| _How much_ it matters                                               | `ai/evaluate.ts` weights | hand-set → **learned** |
...
A tip becomes a feature with a _learnable_ weight; if the tip is wrong, self-play drives the weight
...
- **Per-round shared actions (within-round race):** the board **power actions**, the **QIC/green
  ...
  _once per round_, so within a round it's a sprint to grab the one you want before a rival does —
...
- MCTS with opponent modeling (max-n) literally simulates the opponents racing for the same prize.
  "Keep track of everyone else's path toward it" _is_ the search tree exploring their best
...
- The value net (with the score-margin target, §6.3) learns that being ahead in a race is winning,
  ...
  tempo lead therefore _evaluates worse_, so the AI avoids it — exactly the "don't fall out of the
...
- _One-time claims:_ for each contested asset, _moves-until-I-can-claim-it_ vs
  _moves-until-each-opponent-can_ → a per-asset **lead/deficit** signal.
- _Per-round shared actions:_ current **availability** + whether I can **afford/reach** it before
...
_pre-solves_ the early race optimally — the AI plays the fastest correct line to a contested prize
from move one.
...
_how_ that's encoded:

- **It's emergent from the value target, not a round-1 rule.** Because the value net is trained on the
  **final score margin** (§6.3), not per-round points, it _learns_ that round-1 economic investments
...
- **Economy is multi-dimensional — do NOT proxy it with "Economy track level."** That naive proxy is
  ...
  correctly picks the planet when it's worth more. That _is_ the "not always the Econ track" behavior,
  automatic under a sound value function — the only way to break it is to over-encode a single track.

**Fixed-seed bonus:** round 1 on this board is largely _solved_ by the opening book (§6.1) — the best
...
choosing _which_ buildings and _how many satellites_ (each satellite spends a power token, permanently
...
hex-sets) + a fed-tile choice. Search picks among _bounded candidates_ and decides _when_, instead of
...
- _7-power / few satellites_ → a **fed-efficiency** preference the net learns (overshoot + wasted
  tokens lower the final margin).
- _satellite-scoring final tile_ → the net sees the final-scoring feature (§4) and flips the satellite
  sign, while still balancing token economy across the whole game so it won't cripple itself.
- _don't overload mines; "upgrading a fed building is usually a no-go, sometimes right"_ → the net
  values an upgrade for its _own_ benefits (income/tech/scoring) minus the now-redundant fed-power
...
- **Pass early for tempo (don't reflexively use every action first).** The first player to pass takes
  ...
  claim actions (power / QIC / ship / adv-tech). So passing _before_ spending every action or resource
  is sometimes best — it's really a §7.1 race: passing first _wins_ the race for next round's booster
  and claims. **But it is rare in strong play** — only when the actions/resources you'd forgo are
  worth _less_ than the tempo; with a full hand of useful actions, do **not** pass. The pass-tempo
  feature must be weighed _against_ the value of your remaining actions; the danger is a prior that
...
- **Avoid overcharging power.** Charging when bowls are already full (or will be filled by income)
  ...
  overcharging booster is wasted. Usually avoid — but _soft_ (faction/tempo exceptions), so encode a
...
- **Meta-lesson — audit state/feature completeness.** Both behaviors are only learnable if the AI's
  representation _includes_ the relevant state: **power-bowl contents, pass status, turn/pass order.**
...
- **Justified only when the enabled action beats the burn's loss.** Burning is lossy (~half the
  ...
  **credits/ore at or near zero** — or as **denial** (a §7.1 race / §6.8: burn to take it _before_ the
  opponent). Value/search computes the threshold; do **not** hand-code "burn whenever you can"
  (wasteful) or "never burn."
- **Denial precondition — don't burn to race a ghost.** Burning to grab a power action _before the
  opponent_ only has value if the opponent can actually take it **soon**: either they **already have
...
- **Prefer reaching bowl 3 naturally.** If the opponent is about to build where you can **leech**,
  ...
  to wait for leech or burn now. Burn only when you _can't_ wait (e.g. the opponent can already take
...
The mirror of §7.5: building adjacent to an opponent lets _them_ charge power. Giving them a big
charge early (e.g. 4 power — a full bowl-3 fill — or enough in bowl 2 that they can burn to reach a
power action) hands them _timely_ resources. So when several moves are queued, **order them so the
...
- **The cost is already in margin.** The opponent's power gain is your loss, so the value net counts
  it. What remains is a _sequencing_ problem — the search explores orderings of your moves and prefers
...
- **Cheap feature:** an **opponent-leech-cost** signal per move — how much charge it grants and _how
  usable it is to them right now_ (convertible to a power action they want, directly or via burn this
...
- **Conditional, not a rule.** Don't delay a build you need now just to avoid leech — the value
  ...
  means _when the ordering is otherwise free or the leech is costly_, not always.
...
it _is_ the puzzle); (b) if full re-verification of MCTS moves proves too expensive, fall back to
...
6. **M5 — UI wiring** (§8 steps 1–5): challenge entry, self-contained boot, `ai.worker.ts`,
   human-seat lock, maximum-mode capability gate, and truthful progress/ETA/heartbeat.
...
- Is a full neural net worth it over a very deep MCTS + strong heuristic + opening book for a
  _single_ board? (The net earns its keep only if the tree is too big for search alone.)
...
- How to model / exploit the _specific human population_ (they deviate from self-play). Worth a
...
- **Free-action / resource-conversion combinatorics** (Spend / BurnPower / QIC / brainstone / Gaia)
  blow up per-turn branching; how to prune to _useful_ conversions or fold them into the action they

...
- **Converted offline games are pass-and-play as of v5.48.3.** "Convert to offline pass-and-play"
  now creates one independent local snapshot: every seat is playable on the device, neither history
  overwrites or uploads into the other, and repeated conversion cannot overwrite local progress.
  Previously mirrored Gaia and sidebar-game copies open with their old position but no account lock.

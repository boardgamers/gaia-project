...
deployed site and is the owner's to run. - **Game-bar pulse.**`game-bar.ts`gained`renjuBoardOf`/`renjuMover`/`isMyRenjuTurn`alongside the chess trio, both game lists now embed`renju_board(*)`and subscribe to that
table's Realtime changes, and`game-bar--my-turn`is now`isMyTurn || isMyChessTurn || isMyRenjuTurn`in both`Lobby.vue`and`GameNavPanel.vue`. - **Advantage bar (the real one).** The visual is deliberately the chess meter, copied: the same
text-free 6px strip, white-relative, `role="meter"`with a spoken description in`aria-valuetext`/`title`. The number behind it needed a whole engine, because there is no
...
(victory-by-continuous-four) solver that proves forcing wins far deeper than the main search.`logic/renju-evaluation.ts`is the controller, mirroring`StockfishEvaluator`'s shape.
...
moment a forced win exists and counts down as it is played out. - **Bug found while verifying, fixed here (pre-existing, shipped in #115):**`panel-swipe.ts`called`setPointerCapture`on _pointerdown_, which retargets every later mouse event - including
...
unaffected either way, since it drives selection from pointer events rather than clicks. - **Testing:** viewer **645 passing** (607 baseline, +38:`renju-engine.spec.ts`22,`renju-evaluation.spec.ts`13, plus`RenjuBoard`/`ResearchPanel`/`Lobby`cases), same 2
...
169. ✅ **Analysis mode Phase 5 (docs/lost-fleet/ANALYSIS_MODE_PLAN.md, viewer v5.63.0, 2026-08-16): hazard-stripe visual treatment and the counter's real surfaces (§5).** Commands.vue's sticky header gets a new `&--analysis` variant (full-strength yellow/black `repeating-linear-gradient` at 45°, a solid scrim behind the title text so it stays legible against the stripes) applied to both the mobile sticky bar's `.sticky-bar-title` and, separately, the standalone desktop `#move-title` (which never becomes the sticky bar, so without its own copy of the treatment desktop would have been the one place analysis mode still looked like live play). Tapping either header exits analysis mode, since a map-anchored control can scroll off-screen on mobile. The map itself gets the low-contrast counterpart: `theme.scss`'s shared `.space-map`/`.space-map-canvas` background rule gets a `background-image` stripe layer scoped under a new `.analysis-mode-active` class on Game.vue's own root element, so setup/open-game previews sharing that same base rule are untouched.

     The counter (§4) now has its two real surfaces instead of Phase 2's plain inline readout. The always-visible headline lives in the striped header itself, in the slot the auto-leech dropdown gives up while analysis mode is active (`showAutoLeechSelect` now excludes it, `showAnalysisCounterHeadline` gates the replacement) - compact net deltas plus the feasibility verdict. The full breakdown is a new `AnalysisPanel.vue`, replacing the old plain alert bar at the top of the page: per-resource net/displayed, the power bowl as a before→after pair (not an invented scalar, per §4.3), the feasibility verdict, the two-round-cap note, decision #11's shared-single-use-resource caveat, and the Undo/Reset/Exit controls. Both surfaces read the same `analysisCounter` object, so they can never disagree.

     **A live-browser check (Playwright against the dev server, self-contained `?players=2&lostFleet=1`) caught a real regression no unit test would have**: entering analysis mode during setup made the turn-order banner show a duplicate player circle. Cause: `applySoloRoundFlow`'s `passedPlayers` pre-seed (needed to steer `beginRoundStartPhase`'s own round-1 turn-order fallback ahead of time, added in Phase 4) was being read by `TurnOrder.vue`'s own `passedPlayers` getter as a real "this player has passed" before round 1 had even begun - real games never populate that field before round 1, so the display had no existing guard against it. Fixed by gating `TurnOrder.vue`'s `passedPlayers` getter on `round >= Round.Round1`, which is also a correct, generally-defensive fix independent of analysis mode. Verified visually after the fix: the striped header, the dimmed map, and the full-breakdown panel all render as intended, in both the pre-entry and post-entry states, with the turn-order banner back to showing exactly the real player count.

     **Scope note - the entry/exit button placement:** §5.4 asks for a map-corner (bottom-right) button with the SVG `bounds()` reservation that entails, extending the same clearance system the faction wheel/chart button/terraform swatches already use. This was deliberately **not** built this session: `SpaceMap.vue`'s `bounds()` is a tightly-interdependent hex-clearance calculation with its own dedicated `SpaceMap.spec.ts` coverage, and getting a new bottom-anchored reservation right - correctly co-existing with the round-1+ mobile sticky action bar, which also occupies the bottom of the viewport and has no existing "share the bottom corner" precedent the way `AutoLeechFab.vue`'s premove-bar offset does - was judged a real risk of a subtly-broken layout for a comparatively low-value polish item, given the remaining phases still to land in the same session. The Enter/Exit controls instead stay in `AnalysisPanel.vue` at the top of the page (functionally equivalent, already covered by tests and the live-browser check above); revisiting the map-corner placement is left as a follow-up.

     **Tests:** new `Commands.spec.ts` cases (the auto-leech select hidden and the counter headline shown in both header locations during analysis mode; the stripe classes and the click-to-exit emit); new `AnalysisPanel.spec.ts` (six cases covering the offered/active states, the full breakdown's content, the empty-wallet message, the two-round-cap note, and the decision #11 help text); new `TurnOrder.spec.ts` cases for the `passedPlayers` regression and its fix; a `Game.spec.ts` "Phase 5" case for the `analysis-mode-active` root class. Touched-file viewer specs green (148, adding `Commands.spec.ts`/`AnalysisPanel.spec.ts`/`TurnOrder.spec.ts` to Phase 4's set); the full viewer suite still runs once at the end of the Phase 2-7 session. No engine files touched this phase.

170. ✅ **Analysis mode Phase 6 (docs/lost-fleet/ANALYSIS_MODE_PLAN.md, viewer v5.64.0, 2026-08-16): staleness on re-entry (§3.5) and the leech adjustment stepper (§4.4).** `enterAnalysisMode` no longer just discards a stored line on any `baseMoveCount` mismatch - a new `resolveAnalysisStaleness(seat, stored)` walks §3.5's four-row table in priority order: an unchanged `baseMoveCount` restores silently; a live game that has already advanced past the stored line's own two-round window (`engine.round > stored.baseRound + 1`) clears it with a notice, regardless of who moved; a line whose `moveHistory` gap since it was saved contains none of this seat's own real moves (checked via a new `moveBelongsToSeat(engine, move, seat)`, which parses a move string's leading `p<N>`/faction token exactly the way `engine.ts`'s own `loadTurnMoves` does) auto-replays with a notice; and a gap that does contain this seat's own real moves is held in a new `analysisPendingRestore` field instead of being replayed at all, surfaced in `AnalysisPanel.vue` as an inline Restore-anyway/Discard prompt (mirroring `PremoveBar.vue`'s own mode-switch confirm banner, not a raw `window.confirm`) rather than silently doubling a move the player already made for real. `setAnalysisEntries` now trims the entries it persists down to whatever prefix `replayAnalysisLine` actually applied, so a partially-stale restored line does not keep dragging its dead tail into every later Undo/Reset/save. The `externalData` mid-analysis exit path (already known to keep the persisted line per Phase 1's decision #2) now also sets a plain-language notice explaining why the takeover just closed, closing the "re-anchor and show a notice" gap Phase 1 had left open - re-entering afterward runs straight back through the same staleness table, so "re-anchor" needed no separate mechanism of its own.

     **The leech adjustment stepper (§4.4, decision #12)** answers analysis mode's built-in pessimism about power: opponents never build in the sandbox, so a line never gains the leech charge a real opponent's building would realistically have offered. `AnalysisEntry` is now a union (`AnalysisMoveEntry | AnalysisAdjustEntry`) instead of a single-shape interface - an adjust entry carries just a `charge: number` and is applied on replay as a direct `data.gainRewards([new Reward(charge, Resource.ChargePower)], true, Command.ChargePower)` call (`forced: true` so it can't stall on a brainstone-destination interrupt nothing in the replay loop could ever answer), never a move string, since there is no engine command for "gain power with nobody having offered it." `AnalysisPanel.vue` gets a small stepper (number input + Add button, 1-12, disabled for a non-positive/non-integer value) next to the power bowl row, gated on the sandbox wallet already existing exactly like the rest of the breakdown. No engine changes were needed - `Resource.ChargePower`'s cap-lifting was already covered by the `analysis` flag having nothing to do with it (it moves physical power tokens the sandbox wallet already granted, not one of the three MAX_ORE/MAX_CREDIT/MAX_KNOWLEDGE-capped resources).

     **Tests:** `analysis.spec.ts` gets a `moveBelongsToSeat` describe block and three new `replayAnalysisLine` adjust-entry cases (direct power gain, stops the line on a non-positive charge, mixes move and adjust entries in one line) - 31/31 passing. `Game.spec.ts` gets two new "Phase 6" describe blocks: staleness (unchanged/opponents-moved/own-seat-moved-prompts/discardPendingAnalysisLine/round-advanced-past-cap/the `setAnalysisEntries` trim/`dismissAnalysisNotice`/the `externalData` notice) and the leech stepper (`applyAnalysisAdjust` reflecting in the power bowl, and rejecting a non-positive/non-integer charge) - the whole `Game.spec.ts` file stays green at 57/57. `AnalysisPanel.spec.ts` gets two new describe blocks for the notice banner/pending-restore prompt and the stepper itself - 13/13 passing. Touched-file viewer specs green at 170 (up from Phase 5's 148); the full viewer suite still runs once at the very end of the Phase 2-7 session, per CLAUDE.md's testing table. No engine files touched this phase - `Resource.ChargePower`/`gainRewards`/`Reward` were all already exported and usable as-is.

171. ✅ **Analysis mode Phase 7 (docs/lost-fleet/ANALYSIS_MODE_PLAN.md, viewer v5.65.0, 2026-08-16): the commit path (§6, decision #13) - closing out Phases 1-7 of the whole feature.** A new `committableAnalysisMoves(origin, entries, seat, baseRound)` in `analysis.ts` is the affordability gate: it strips every `adjust` entry out of the line first (never merely skipping-but-counting it - a move after one is only committable if it is genuinely affordable **without** the leech it assumed), replays the move-only line completely fresh, and cuts the returned prefix at the first point that goes infeasible (reusing `computeAnalysisCounter`'s own `infeasibleFromMove`) or wherever the replay itself stops applying, capped at a new `MAX_COMMITTABLE_MOVES = 4` (1 live move + `PremoveBar.vue`'s own 3-row queue limit - not an arbitrary number, just what the existing premove machinery already allows). A line that never left setup commits every successfully-replayed move as-is, since setup carries no cost to check. `Game.vue`'s `analysisCommittableMoves` getter layers two more, purely UI-side caps on top: self-contained/hot-seat play only ever gets move 1 (§6: "Premoves are hosted-only. In self-contained/offline play, offer move 1 only"), and hosted play is further capped by however much room is actually left in this seat's REAL premove queue right now (`3 - <that seat's already-queued count>`), so committing a line can never try to push the real queue over its own limit just because the analysis line itself would have allowed more. `commitAnalysisLine()` clears the persisted line (a new `clearAnalysisLine(seat)`, removing the storage key outright rather than persisting an empty line - unlike a normal exit, which keeps the line per decision #2, a commit has nothing left worth coming back to), exits analysis mode first (back to the exact real pre-commit state - nothing else could have changed it while analysis mode was active), then dispatches move 1 through the exact same `"move"` pipeline a manually-typed turn already uses (`addMove`'s own dispatch) and queues the rest via the exact same `queuePremove` dispatch `PremoveBar.vue`'s own confirm button already uses - reusing that existing, already-tested machinery end to end rather than reimplementing any of its hosted-round-trip/self-contained-persistence behaviour locally, the same "dispatch first, then restore" order `queueCurrentPremove`/`cancelPremoveMode` already use. `AnalysisPanel.vue` gets a green "Commit" button in the header action row (next to Undo/Reset/Exit), shown whenever the line has anything in it at all, disabled with a "Nothing committable yet" label rather than hidden when nothing qualifies, and labelled "Commit this move" vs. "Commit N moves (1 live + N-1 queued)" depending on the count.

     **A real bug found and fixed while building this, before it ever shipped:** the first version of `committableAnalysisMoves` passed a bare `initialWallet: null` into `replayAnalysisLine` unconditionally, on the assumption that the fresh, adjust-stripped replay would grant its own wallet the same way a setup-phase entry's lazy grant already does. It doesn't - `replayAnalysisLine`'s lazy grant only fires on the TRANSITION into `Phase.RoundMove` (`!wasRoundMove && copy.phase === Phase.RoundMove`), which never happens for a line whose `origin` already starts in `Phase.RoundMove` (the overwhelmingly common case - anything that isn't a setup-phase entry). Passing `null` there left `wallet` permanently null, which the function's own `!wallet` branch reads as "setup line, nothing has a cost" - so it was treating every round-flow line as free, offering an unaffordable move as committable. Caught immediately by a `analysis.spec.ts` test built exactly to exercise this (zero real credits/ore, a move that only works via the sandbox top-up) failing with the move present in the result instead of absent. Fixed by mirroring what `enterAnalysisMode` already does for the same split: eagerly grant a fresh wallet up front, on a fresh clone, whenever `origin.phase === Phase.RoundMove`, and only fall through to the lazy-grant path for a genuine setup-phase entry.

     **Live-browser check (Playwright against the dev server, self-contained `?players=2&lostFleet=1`), the same discipline Phase 5's own check used:** fast-forwarded to round 1 via a directly-injected engine (`gameVm.handleData(new EngineCtor(SETUP_MOVES))`), entered analysis mode, applied two moves, and confirmed visually and by direct click that every new Phase 6/7 surface renders and works together correctly at once - the staleness notice banner (with a working dismiss `✕`), the own-seat pending-restore prompt (Restore anyway correctly cleared it and restored the entry, verified via a real click), the leech-adjustment stepper, the feasibility verdict, and the green Commit button, all inside the still-striped Phase 5 header/map. A real click on the rendered Commit button correctly fired the full click → emit → `commitAnalysisLine` → `exitAnalysisMode` chain end to end (`analysisMode` flipped back to `false`, the board cleanly reverted to the real pre-analysis state with no leftover sandbox inflation) - the one console `AssertionError` that followed is the self-contained launcher's own independently-tracked internal game correctly rejecting a move against a differently-seeded game than the one this test manually injected into the DISPLAYED engine only, a test-harness artifact of bypassing real UI setup rather than a defect in this phase's code (the dispatch pipeline itself - `addMove`'s `"move"`/`"queuePremove"` dispatches - is pre-existing, already-tested infrastructure this phase reuses rather than touches).

     **Tests:** `analysis.spec.ts` gets a `committableAnalysisMoves` describe block (full-line commit, adjust-entry exclusion, the real bug's own regression case, a setup-only line, empty-line handling, the `MAX_COMMITTABLE_MOVES` constant) plus a `clearAnalysisLine` persistence case - 38/38 passing. `Game.spec.ts` gets a "Phase 7" describe block (self-contained caps at move 1, the persisted line is cleared not kept, doing nothing when nothing is committable, hosted mode dispatching move 1 live plus queuing the rest, and capping queueing by the real queue's own remaining room) - the whole file stays green at 62/62. `AnalysisPanel.spec.ts` gets a describe block for the Commit button's three label states and its disabled state - 16/16 passing. Touched-file viewer specs green at 185 (up from Phase 6's 170). **This closes Phases 1-7 of Analysis Mode entirely** - the full viewer test suite now runs once, for real, at the very end of this session, per CLAUDE.md's testing table and this plan's own §8.

172. ✅ **Analysis mode: entry is now offered off-turn (any locked seat, any phase, any time), plus §5.4's map-corner button - both from a live owner report (viewer v5.67.0, 2026-08-16).** An owner playing a live 3p Silent Auction game ("Ivory Beacon") reported the entry button missing during the secret-bid round, even though it was genuinely their move to make. First fix: `analysisOffered` gated entirely on `canPlay`, which is only ever true for whichever seat `engine.playerToMove` happens to point at - but a simultaneous sealed-bid round (Silent Auction/Preference Split) isn't gated by turn order from the player's point of view at all (`SealedBidPanel.ts`'s own doc comment: "canPlay ... is only ever true for the one seat the engine happens to point at, so everybody else would have no way to submit" - `Commands.vue`, what `canPlay` actually gates, isn't even rendered during one of these rounds). The owner then asked for the same thing more generally - the button available off-turn everywhere, not just during a sealed-bid round - and it turned out the sealed-bid case was one instance of a pattern the underlying mechanics already supported for free: `applySoloRoundFlow` (§2.5/§3.1) forces the clone's `turnOrder`/`currentPlayer` to the entering seat outright the moment it reaches `Phase.RoundMove`, regardless of who the real engine's `playerToMove` was at entry, and `grantSandboxWallet` grants directly to `players[seat].data` with no turn dependency either - the gate was always the only thing that was turn-hostile. `analysisOffered` now reads `myLockedSeat !== undefined ? true : canPlay` - unconditional for any locked (hosted, real-account) seat, superseding the narrower `sealedBidPhase` special case entirely (removed, since it's now just one case the general rule already covers); hot-seat/pass-and-play keeps its existing `canPlay` gate, which was already unconditionally true there (no "my seat" identity to be off-turn from in the first place). Composing a move or bid once inside already worked regardless of turn - Phase 4 had already nulled the real sealed-bid backend and made `SealedBidPanel.mySeats` render every seat's form during analysis mode, and setup pass-and-play already walks every seat's turn (decision #7) - so the only thing ever missing, both times, was this one gate.

     **The owner also asked for §5.4's map-corner button**, deliberately left unbuilt in Phase 5 (PROGRESS.md #169's scope note) over risk to `SpaceMap.vue`'s hex-clearance geometry - built now that there's no remaining-phases time pressure. A plain magnifying glass (new `#analysis-magnifier` def in `Definitions.vue`, geometry rather than an external asset so its stroke can follow `currentColor`), positioned bottom-right of the hexmap mirroring the chart-history icon's existing top-right placement (same inset/scale convention, anchored to `bounds.bottom` instead of `bounds.top`), toggling both directions through one `analysis-toggle` emit Game.vue resolves against its own `analysisMode` state (`toggleAnalysisMode()`). Neutral (`--ui-surface`/`--ui-border`) while merely offered, switching to the same `--ui-warning-*` tokens the striped header/`AnalysisPanel.vue` already use once active - ties the corner control into the feature's established visual language rather than introducing a fourth one. `SpaceMap.vue`'s `bounds()` gets a second, independent right-edge reservation for it: a new `bandMaxXFromBottom` (mirroring the existing top-band `bandMaxX`) checks hexes near the bottom-right corner specifically, since the two bands never overlap for any board shape this game has. This is purely additive to the existing panel button and the striped-header tap-to-exit from Phase 5 - all three now coexist, matching §5.4's own reasoning for keeping more than one exit path ("on mobile the map is often scrolled and a map-anchored control can be off-screen when you want out"), and with the off-turn widening above, all three are now available off-turn too.

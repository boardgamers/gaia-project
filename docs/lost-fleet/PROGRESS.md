...
**Done 2026-07-11 round 3: chat/game-menu panels default open, iOS-style settings toggles, a
"new settings" badge.** Owner: both side panels should be on by default with a settings-menu
toggle to turn them off, styled as "a visible toggle like on iphone" (including dark mode, which
should read as one stable "Dark mode" label with a toggle, not text that swaps between "Dark
mode"/"Light mode"), plus a way to let users know when new settings options ship without expecting
anyone to read the changelog. Brainstormed and implemented as follows (owner said "as you see fit"
rather than prescribing the mechanism):

- **New `SettingsToggle.vue`** - a reusable iOS-style on/off row (real `<input type="checkbox">`
  under the hood for accessibility, driven track/knob spans for the look) for use inside any
  `<b-dropdown>` settings menu. Deliberately not a `b-dropdown-item-button` - those auto-close the
  dropdown on click, which is wrong for a switch you might want to flip more than one of in a row;
  `@click.stop` on the root keeps the dropdown open, iOS-settings-sheet style.
- **New `settings-notice.ts`** - the "new settings" mechanism, deliberately simple: one exported
  `CURRENT_SETTINGS_VERSION` constant to bump whenever a new option ships, `hasUnseenSettings()` /
  `markSettingsSeen()` compare it against a `localStorage`-persisted "last seen" version. Both
  Lobby.vue's and HostedBar.vue's own settings gears share this one flag (opening either counts as
  "seen"), each showing a small pulsing red badge on the gear icon until then, cleared via each
  dropdown's own `@shown` event - fires once, never again once seen, per the owner's explicit
  requirement.
- **Dark mode is now a toggle switch** (`SettingsToggle` bound to `isDarkMode`) in both Lobby.vue
  and HostedBar.vue, replacing the old text-swapping `"Dark mode"/"Light mode"` item.
- **ChatNotesPanel and GameNavPanel now default OPEN on desktop** (own judgment call: mobile still
  defaults closed - both become full-screen overlays there, which would otherwise cover the board
  the instant a phone user opened the app; this wasn't explicitly restated in this round's request
  but matches every other change in this whole layout thread being desktop-only). Both gained
  persisted `open` preferences (`loadChatOpenPreference`/`saveChatOpenPreference`,
  `loadGameNavOpenPreference`/`saveGameNavOpenPreference`, both defaulting to
  `window.innerWidth >= 768` when nothing is stored yet) and are now controlled from HostedBar's
  settings dropdown via two more `SettingsToggle` rows ("Chat panel", "Game menu").
- **ChatNotesPanel's own floating bubble toggle removed entirely**, for consistency with
  GameNavPanel (which already had no floating toggle of its own from the previous round) - now that
  both panels are on by default and controlled from one shared settings surface, a second/separate
  toggle affordance was redundant. The in-panel back/close (×) buttons still work as a quick dismiss
  without opening settings; closing that way also persists the preference. This also DROPPED the
  unread-message badge feature (there's no floating button left to put it on) - a known, accepted
  trade-off given this round's scope, not an oversight.
- **AutoLeechFab's earlier desktop CSS offset (dodging the now-removed floating chat toggle)
  reverted back to its simpler original position**, since there's nothing left in that corner to
  dodge.
- **hosted.ts** wires `bar.chatOpen`/`bar.gameNavOpen` into each fresh per-game `HostedBar` mount
  from the two panels' own live `open` state (`nav` persists across in-app game switches;
  `chatNotes` doesn't, so its watcher is set up fresh each `mountGameInstance` call alongside the
  existing one that drives the `chat-notes-open` CSS class), and the two new `"toggle-chat"`/
  `"toggle-game-nav"` events flip the real source of truth and persist it.
- New `SettingsToggle.spec.ts` (3/3), `settings-notice.spec.ts` (4/4); `ChatNotesPanel.spec.ts`,
  `HostedBar.spec.ts`, and `GameNavPanel.spec.ts` substantially rewritten for the new default-open/
  toggle-switch behavior (all passing). Full `pnpm test` baseline unchanged (same known ~31
  pre-existing failures, zero new ones - confirmed by running every touched spec file individually
  plus the full suite).
- **Not visually verified live**: same standing limitation as the rest of this layout work - every
  piece here is hosted-mode-only (Lobby.vue's and HostedBar.vue's settings dropdowns), and this
  environment has no real signed-in session to open them in. Verified instead via clean
  `tsc --noEmit`, the test suites above (including real DOM assertions on the toggle's checked
  state and emitted events), and a live self-contained-mode smoke check confirming the shared
  bundle still compiles and boots cleanly. **Please sanity-check on the real deploy**: that both
  panels actually appear open on a fresh desktop sign-in, that the three toggle switches visually
  match "an iPhone toggle," and that the new-settings badge appears once (bump
  `CURRENT_SETTINGS_VERSION` in `settings-notice.ts` to re-trigger it for testing) and then stays
  gone after opening settings once.

**Done 2026-07-11 (desktop-only, hosted mode): dark-mode fix, shared GameBar component, global
game-menu setting, button/overlap polish.** Owner feedback round 2 on the desktop layout work
above:

- **Dark mode was rendering pure black instead of dark gray - root-caused and fixed.**
  `frontend.scss`'s dark-mode trick inverts `:root`'s own `background: #e3e3e1` to a dark gray, but
  Bootstrap's `body { background-color: #fff }` paints straight over it before the filter ever gets
  a chance to matter - the whole page was really inverting from white->black, not from the intended
  `#e3e3e1`->dark-gray. Fixed by repeating the same pre-invert value on `body` inside the
  `:root[data-theme="dark"]` block. Verified live (self-contained mode, filter mechanism is
  app-wide): body's computed background now matches `rgb(227, 227, 225)` instead of staying white,
  screenshot confirms uniform dark gray instead of black.
- **GameNavPanel (the left in-game menu) redesigned to share code with Lobby.vue, not just look
  similar.** Owner: "should be identical, so any changes in either effect both." Extracted:
  - `game-bar.ts` - pure, no-`this` functions (`isMyTurn`, `isMyGame`, `sortGames`, `claimedSeats`,
    `auctionLabel`, `isTestGame`, `summaryForGame`, `moveAge`/`formatMoveAge`, `playersWithSummary`,
    `playerRows`, `factionInitial`, `playerBarTitle`, `playerPresence`) - both Lobby.vue's own list
    sorting/filtering AND the shared row component below call these, so there is exactly one
    implementation of "what counts as my turn," "how are games sorted," etc. in the whole app.
  - `GameBar.vue` - the actual per-game row markup (round/seats badge, title+tags+delete-test-game
    button, summary/age, player avatars with presence+score), moved verbatim out of Lobby.vue.
    Deliberately presentational: computes its own `href`, but leaves click interception (Lobby's
    swipe-cancel, GameNavPanel's in-app switch) to the caller via `@click.native`, and emits
    `delete-test-game` for the one action that needs to stop propagation. Its own `.game-bar*` CSS
    is **global, not scoped** (with a comment explaining why) - Vue's scoped CSS attaches to
    elements based on which component's template rendered them, so a scoped block in GameBar.vue
    could never reach Lobby.vue's own wrapping `<b-list-group-item class="game-bar">`, and vice
    versa; global class names are the one thing both callers can actually share.
  - Lobby.vue now renders `<GameBar>` instead of ~70 lines of inline markup; GameNavPanel.vue does
    the same (now via a real `<b-list-group>`/`<b-list-group-item class="game-bar">` wrapper
    matching Lobby's structure, not its own bespoke row), and gained `deleteMyTestGame` (same
    `delete_my_test_game` RPC Lobby.vue already had) and presence (shared from `hosted.ts`'s
    already-tracked `emitter.store.state.presence`, same cross-instance-assignment pattern as
    `chatNotes.presenceState` - no second Presence channel).
  - New `GameBar.spec.ts` (6/6). `Lobby.spec.ts` needed one call-site update (`playerBarTitle` moved
    off the Lobby instance onto the shared module) and otherwise passed unchanged, confirming the
    extraction preserved exact rendered output. `GameNavPanel.spec.ts` rewritten for the new
    rendering path (9/9).
- **The left menu is now a persisted, global on/off preference (owner: "not to be a per game
  specific setting"), not per-session UI state.** Its own floating hamburger toggle button is gone;
  toggling now happens from HostedBar.vue's ⚙ settings dropdown ("Show game menu"/"Hide game menu",
  same slot as the Dark mode toggle), backed by `localStorage` (`GameNavPanel.vue`'s
  `loadGameNavOpenPreference`/`saveGameNavOpenPreference`) so it stays on/off consistently across
  every game and every future visit instead of resetting on each fresh mount. `hosted.ts` wires
  `nav` (persists across in-app game switches) into each fresh `bar` instance's `gameNavOpen` data
  and a `toggle-game-nav` handler. New `HostedBar.spec.ts` cases (6/6) and `GameNavPanel.spec.ts`
  cases for the persistence itself.
- **Desktop off-turn auto-leech FAB no longer overlaps ChatNotesPanel's floating chat toggle.**
  Both sat fixed in the same bottom-right corner on desktop (mobile had already been fixed for a
  similar overlap between the auto-leech dropdown and the chat toggle, a different code path -
  `AutoLeechFab.vue` is the OFF-TURN floating control, not the on-turn sticky-bar dropdown that
  earlier fix touched). Shifted `AutoLeechFab.vue`'s desktop `right` offset left by the chat
  toggle's own offset + width + a gap so they sit side by side instead of stacking. Verified via the
  CSS arithmetic (chat toggle's left edge vs. FAB's new right edge, 0.6rem clear) rather than live
  pixel measurement - this component requires hosted-mode's seat-locking mechanism, which needs a
  real signed-in session to exercise.
- **Chat panel's "Receiving push notifications" toggle now looks like a real button.** Was
  borderless/transparent muted text, easy to mistake for a passive status label - same treatment
  already given to HostedBar's push bell (visible border, background, hover/press shadow feedback).
  `ChatNotesPanel.spec.ts` unaffected (10/10 still passing - pure CSS change).
- **Returning to the main screen now always lands on My games, not Lobby.** `Lobby.vue`'s
  `created()` default changed from `activeTab = "open"` to `"mine"`. `Lobby.spec.ts`'s default-tab
  test updated to match (My games first, Lobby/Active/Finished all still reachable via their tabs).
- **Not visually verified live**: all of the above except the dark-mode fix are hosted-mode-only
  (need a real signed-in session against the live Supabase project, which this environment has no
  credentials for). Verified instead via: clean `tsc --noEmit`, the full `pnpm test` suite (same
  known ~31 pre-existing failures as the clean baseline, zero new ones - confirmed by running each
  touched spec file individually), and reasoning through the DOM/CSS changes directly. **Flagging
  again for the owner to sanity-check on the real deploy**, in particular: the settings-menu game-
  menu toggle persisting correctly across a real reload, GameBar rendering identically in both
  Lobby and the in-game menu side by side, and the AutoLeechFab/chat-toggle spacing on a real
  desktop off-turn view.

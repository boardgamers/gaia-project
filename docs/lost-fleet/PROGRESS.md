...
137.  ✅ **"Preference Split Auction" — a new simultaneous, secret, budget-limited faction-selection
      ...
      range 1–999, **default 10 points per player** — 20/30/40 at 2/3/4). It shipped 4-players-only
      ...
      **Tests:** ~30 algorithm cases (incl. the owner's near-tie example, both "pays more than they
      ...
      cases, 4 new-game cases and 4 CreateGame cases. **The 2/3-player generalization's own tests
      were written but NOT executed** — owner instruction, verbatim: _"Don't run anymore tests"_.
      Everything up to and including the no-cap pricing change was verified green (engine 669
      passing, viewer 777 passing with the one pre-existing SetupPreview failure). Engine suite (excluding
...
pin_preference_split_budget_search_path` (the one function in the set without a pinned
      ...
      **The `resolve-automation` Edge Function is NOT redeployed, and this is a pre-existing repo
      problem, not something this feature introduced.** Its committed `_shared/engine.bundle.js` was
      rebuilt here for the new `preferenceBid` command, and pushing it to master did fire
      `.github/workflows/supabase-deploy-function.yml` — which failed, as has _every_ run of that
      workflow since at least 2026-07-27, on `unexpected list functions status 401: Unauthorized` at
      its very first step. The repo secret `SUPABASE_ACCESS_TOKEN` is expired; the live function is
      still version 3 from 2026-07-19. Deploying it through the MCP instead is impractical (the
      bundle is 583 KB). **Impact while it stays stale is contained**: `resolveOneAutomatedTurn`
      wraps its `new Engine(...)` in a try/catch and returns `{outcome: "replay-failed"}`, so for a
      Preference Split game it is a clean no-op — no premove is deleted, no failure notification is
      raised, nothing is corrupted, and every other game is unaffected. Only _fully offline_
      premove/auto-leech execution is unavailable for this variant; the client-side fast path still
      runs whenever anyone has the game open. Fix: rotate the secret, then re-run the workflow.

...
| Shifted-sector setup geometry | per player count (2/3/4) | encode the offset-placement layout (rulebook p.4-5). **CODED & verified** in `lost-fleet-map.ts` `lostFleetSectorCenters()` + `findInterspaceHoles()` + `findDeepSpaceNotches()` (Chunk 5): 6/8/10 single-hex Interspace holes, 6/8/8 Deep Space triangle notches, 0 overlap. **Full `Grid<GaiaHex>` assembly CODED** in `lost-fleet-board.ts` `generateLostFleetBoard()` (Chunk 6) — sector tiles + Interspace + Deep Space all placed, seed-deterministic. The `GaiaHex` addressing bug that blocked wiring this into `SpaceMap` is now **FIXED** (Chunk 7a, `sectorCenter` stamped in `Sector.create()`); still standalone — not yet wired into the real `SpaceMap`/`moveInit`, that's Chunk 7b (see PROGRESS.md "Next actions" #2 for the additional `generate()`/`load()`/`parse()` surface area found while scoping it) | ● CODED (standalone) |

...
export type StoredOfflineGame = OfflineGameSave & {
  ...
  createdAt: string;
  /**
   * Set only on a record that was an automatically maintained copy of a hosted (online) game: the
   * online game's id. Optional, so every record written before the mirror existed - and every
   * ordinary pass-and-play game - stays valid without a migration.
   */
  mirrorOf?: string;
  /**
 ...
 * id, and `writeStoredOfflineGame`, which refuses a missing one. A mirror-style caller derives its
 * id from the online game's id rather than minting one per save, so the same call has to serve
 * both the very first sync and every later one.
 *
 * Takes already-serialized engine data rather than an `Engine`: a hosted host hands its state out
 * as plain JSON, and re-hydrating an Engine only to serialize it straight back again would be pure
 * waste. Any half-composed `pendingMove` on an existing record is dropped

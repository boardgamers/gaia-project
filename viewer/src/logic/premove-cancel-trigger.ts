...
// first". Imported identically by host.ts's client fast-path and by
// supabase/functions/resolve-automation/logic.ts (which imports this .ts source directly, the
// same way it already does for premove-resolver.ts), so online/offline evaluation can never drift.
//

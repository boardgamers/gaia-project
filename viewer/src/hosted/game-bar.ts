 ...
 * in-game menu (owner request: the two must render identically, and any change to one must apply
 * to both automatically) - Lobby.vue uses these directly for sorting/filtering the raw game list
 * before it ever reaches a component, and GameBar.vue (the shared per-row presentational component)
 * uses the same functions for what it displays, so there's exactly one implementation of "what does
 * this game's row look like" in the whole app.
 */

-- Restricts game creation to the app admin (only), mirroring 0006's
-- delete_game admin check. Everyone else already has the "+ New game" link
-- and the create-game screen hidden client-side (Lobby.vue/CreateGame.vue),
-- but that's UI-only - this is the actual enforcement point, since
-- create_game was previously granted to every authenticated user.
...
begin
  ...
  if lower(coalesce(auth.jwt() ->> 'email', '')) <> v_admin_email then
    raise exception 'only the admin can create games';
  end if;

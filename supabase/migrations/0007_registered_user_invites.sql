-- Replaces email-typed invites with a "pick from already-registered users"
-- flow: the create-game screen now queries list_registered_users() to let
-- the host pick players by name instead of typing an email address that may
-- not match an account. create_game's p_invites shape changes from
-- {email, seat, display_name} to {user_id, seat, display_name}; a typo can
-- no longer orphan a seat since user_id always comes from an already-picked,
-- already-registered row. invited_email is still populated (resolved
-- server-side from auth.users) so existing email-fallback matching
-- (host.ts's mySeats, is_game_member) keeps working unchanged. Duplicate
-- user_ids across seats are still allowed (test games: the host plays every
-- seat), matching 0003's removal of the per-game email-uniqueness
-- constraint.
...
revoke execute on function public.list_registered_users() from public, anon;
